...
@section('content')
    <div class="container mt-4">
                    ...
                    <table class="table">
                        ...
                        <tbody>
                            @foreach($order->orderItems as $item)
                                <tr>
                                    <td>{{ $item->productVariant->product->name }} {{ $item->productVariant->brand }} - {{ $item->productVariant->attribute }}</td>
        ...
        <div class="card mb-4">
            <div class="card-body">
                @php
                    // Determine current admin/staff user (middleware may set admin_user_id in session)
                    $currentUser = null;
                    if (session()->has('admin_user_id')) {
                        $currentUser = \App\Models\User::find(session('admin_user_id'));
                    } else {
                        $currentUser = auth()->user();
                    }
                @endphp

                {{-- Staff (nhân viên) can only confirm orders: show a single button when order is pending --}}
                @if($currentUser && $currentUser->role == 'staff' && $order->status == 'pending')
                    <form action="{{ route('admin.orders.update-status', $order->id) }}" method="POST" class="d-inline me-2">
                        @csrf
                        @method('PATCH')
                        <input type="hidden" name="status" value="confirmed">
                        <button type="submit" class="btn btn-success">Xác nhận (Nhân viên)</button>
                    </form>
                @endif

                {{-- Admin (hoặc staff nếu permitted) can update to any allowed status --}}
                @if($currentUser && $currentUser->role == 'admin' && $order->status != 'completed' && $order->status != 'cancelled')
